<template>
  <div class="max-w-4xl mx-auto pt-30">
    <ElUpload ref="uploadRef" class="upload-demo" action="http://localhost:8686/file/upload" :on-remove="handleRemove"
      :on-progress="handleProgress" :on-change="handleChange" :on-success="handleSuccess" :on-error="handleError"
      :file-list="fileList" :auto-upload="false" :show-file-list="true" drag multiple>
    ...
    <div class="">
      ...
      <ElTable :data="fileTableData" class="mt-4" :cell-style="{ textAlign: 'center' }" stripe>
        ...
        <ElTableColumn label="进度" width="120">
          <template #default="scope">
            <ElProgress :text-inside="true" :stroke-width="15" :percentage="scope.row.num" />
          </template>
        </ElTableColumn>
        <ElTableColumn label="操作">
          <template #default="scope">
            <ElButton type="danger" class="el-icon-close" size="mini" :disabled="!(scope.row.status === 'uploading')"
              title="取消上传" @click="cancelUpload(scope.$index, scope.row)" />
            <ElButton type="danger" class="el-icon-delete" size="mini" title="删除文件" @click="delFile(scope.$index)" />
          </template>
        </ElTableColumn>
