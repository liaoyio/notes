...
使用 `v-for` 循环生成列表时，应该给每个列表项一个稳定唯一的`key`，这有利于在列表数据变动时，尽量少的删除、新增、改动元素。


<script>
import FreezeDatas from './优化/1. 使用冻结对象/FreezeDatas.vue'
import VueModel from './优化/3. 非实时绑定的表单项/index.vue'
export default {
  components: { 
    FreezeDatas,
    VueModel
